...
const ChatComponent = ({ currentUser, recipientUser, onClose }) => {
  ...
  if (!chatId) return;
  try {
    console.log('Attempting to delete message:', { chatId, messageId, userId: currentUser._id }); // Debug log
                      ...
                      >
                        ...
                        {message.sender === currentUser._id && (
                          <div className="flex items-center space-x-1">
                            ...
                            <div className="relative">
                              <button
                                className={`p-1.5 rounded-full transition-all duration-200 bg-black-500 text-black hover:bg-blue-600 focus:outline-none ${
                                  activeDropdown === message._id ? "scale-110 ring-2 ring-blue-300" : ""
                                }`}
                                onClick={() => toggleDropdown(message._id)}
                              >
                                <ThreeDotsVertical className="w-4 h-4 text-white" />
                              </button>
                              {activeDropdown === message._id && (
                                <div className="absolute right-0 bottom-full mb-2 w-24 bg-white rounded-lg shadow-lg z-10 border border-blue-100">
                                  <button
                                    onClick={() => handleEditMessage(message)}
                                    className="w-full text-left px-3 py-2 text-sm text-blue-600 hover:bg-blue-50 transition-colors duration-200"
                                  >
                                    Edit
                                  </button>
                                  <button
